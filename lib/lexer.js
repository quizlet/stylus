...
Lexer.prototype = {
  ...
  /**
   * Converts a media shorthand (@x) to a media string.
   */

	tomedia: function(dir, min, max) {
		var str = '';
		dir = dir.split('@').pop().trim();
		
		if (min) {
			var minvalue = min.trim();
			if (Number(minvalue)) {
				minvalue += 'px';
			}
			str += ' and (min-' + dir + ': ' + minvalue + ')';
		}

		if (max) {
			var maxvalue = max.trim();
			if (Number(maxvalue)) {
				maxvalue += 'px';
			}
			str += ' and (max-' + dir + ': ' + maxvalue + ')';
		}
  ...
  /**
   * @media shorthand handler.
   * (\d+ <)? '@x' (< \d+)?
   */

	mediashorthand: function() {
		var captures;
		// Terribly ugly, but necessary for the way the lexer is set up.
		if (captures = /^(((([0-9a-zA-Z_-]+[ \t]*<[ \t]*)@(width|height)([ \t]*<[ \t]*[0-9a-zA-Z_-]+)?)|(([0-9a-zA-Z_-]+[ \t]*<[ \t]*)?@(width|height)([ \t]*<[ \t]*[0-9a-zA-Z_-]+)))( and )?){1,2}/.exec(this.str)) {
			this.skip(captures);
			var statements = captures[0].split('and');
			var mediaval = 'screen';
			for (var i = 0; i < statements.length; i += 1) {
				var parts = statements[i].split('<');
				if (parts.length == 3) {
					mediaval += this.tomedia(parts[1], parts[0], parts[2]);
				} else if (parts[0].indexOf('@') != '-1') {
					mediaval += this.tomedia(parts[0], null, parts[1]);
				} else {
					mediaval += this.tomedia(parts[1], parts[0]);
				}
			}
			return new Token('media', mediaval);
		}
	},
