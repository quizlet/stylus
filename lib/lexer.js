...
Lexer.prototype = {
   ...
   */
	...
	mediashorthand: function() {
		var captures;
		// TODO(michellebu): Add support for variables.
		if (captures = /^([0-9]+[a-z]+[ \t]*<[ \t]*)?@x([ \t]*<[ \t]*[0-9]+[a-z]+)?/.exec(this.str)) {
			this.skip(captures);
			var min = captures[1] ? captures[1].split('<').pop().trim() : null;
			var max = captures[2] ? captures[2].split('<').unshift().trim() : null;
