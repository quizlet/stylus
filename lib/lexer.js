...
Lexer.prototype = {
   ...
   */
	...
	tomedia: function(min, max) {
		var str = 'screen';
		str += min ? (' and (min-width: ' + min + ')') : '';
		str += max ? (' and (max-width: ' + max + ')') : '';
		return str;
	},
   ...
   */
	...
	mediashorthand: function() {
		var captures;
		// TODO(michelle): Add support for variables.
		if (captures = /^([0-9a-zA-Z_-]+[ \t]*<[ \t]*)?@x([ \t]*<[ \t]*[0-9a-zA-Z_-]+)?/.exec(this.str)) {
			this.skip(captures);
			var min = captures[1] ? captures[1].split('<').shift().trim() : null;
			var max = captures[2] ? captures[2].split('<').pop().trim() : null;
			return new Token('media', this.tomedia(min, max));
		}
	},
