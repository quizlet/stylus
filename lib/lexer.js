...
Lexer.prototype = {
   ...
   */
	...
	tomedia: function(dir, min, max) {
		...
		if (min) {
			var minvalue = min.split('<').shift().trim();
			if (Number(minvalue)) {
				minvalue += 'px';
			}
			str += ' and (min-' + dimension + ': ' + minvalue + ')';
		}

		if (max) {
			var maxvalue = max.split('<').pop().trim();
			if (Number(maxvalue)) {
				maxvalue += 'px';
			}
			str += ' and (max-' + dimension + ': ' + maxvalue + ')';
		}
   ...
   */
	...
	mediashorthand: function() {
		...
		if (captures = /^([0-9a-zA-Z_-]+[ \t]*<[ \t]*)?@(x|y)([ \t]*<[ \t]*[0-9a-zA-Z_-]+)?( and ([0-9a-zA-Z_-]+[ \t]*<[ \t]*)?@(x|y)([ \t]*<[ \t]*[0-9a-zA-Z_-]+)?)?/.exec(this.str)) {
			this.skip(captures);
			var mediaval = 'screen' + this.tomedia(captures[2], captures[1], captures[3]);
			if (captures[4]) {
				mediaval += this.tomedia(captures[6], captures[5], captures[7]);
			}
