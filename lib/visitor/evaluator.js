...
Evaluator.prototype.visitMedia = function(media){
  ...
  if (variables.length > 1) {
		for (var i = 0; i < variables.length - 1; i += 1) {
			...
			var name = parts.shift().split('-').pop();
			var variable = parts.pop();
			var value = variable;

			// Look up variable.
			var query = this.lookup(variable);
			if (query && query.first.val) {
				value = query.first.type ? query.first.val + query.first.type : query.first.val;
			}
			console.log(name);
			if (Number(value) && (name == 'width' || name == 'height')) {
				value += 'px';
			}
