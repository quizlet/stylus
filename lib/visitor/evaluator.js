...
Evaluator.prototype.visitMedia = function(media){
  ...
  var valuestring = query ? query.first.string : media.val.toString();

  // Check for variables in the media string.
  var variables = valuestring.split(')');
  if (variables.length > 1) {
		for (var i = 0; i < variables.length - 1; i += 1) {
			// Replace variable names with variable values.
			var variable = variables[i].split(' ').pop();
			var value;
			// max-width and height and inclusive, min- is exclusive so we add 1.
			var exclusive = 0;
			if (variables[i].indexOf('min') != -1) {
				exclusive = 1;
			}

			// Look up variable.
			var query = this.lookup(variable);
			if (query && query.first.val && query.first.type) {
				value = (query.first.val + exclusive) + query.first.type;
			} else {
				// Try to parseInt to determine if we need px.
				if (value = parseInt(variable)) {
					value += 'px';
				}
				// Increment if min.
				if (exclusive) {
					value = value || variable;
					value = value.replace(/(\d+)/, function(n) { return Number(n) + 1; });
				}
			}
			// Replace the value.
			if (value) {
				valuestring = valuestring.replace(variable, value);
			}
		}
	}
	media.val = new nodes.Literal(valuestring);
  return media;
};
