...
Evaluator.prototype.visitMedia = function(media){
  ...
  var valuestring = media.val.toString();
  ...
  if (variables.length > 1) {
		for (var i = 0; i < variables.length - 1; i += 1) {
			...
			if (!!variable) {
				...
				var query = this.lookup(new nodes.Literal(variable));
				if (query) valuestring = valuestring.replace(variable, query.first.string);
			}
		}
	}
